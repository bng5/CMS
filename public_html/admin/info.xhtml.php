...
// {
    $no_poromision = TRUE;
    if (!$consulta_lengs = $mysqli->query("SELECT l.id, nombre, nombre_nativo, codigo, dir FROM `lenguajes` l LEFT JOIN `lenguajes_nombres` ln ON l.id = ln.id AND ln.leng_id = {$_SESSION['leng_id']} ORDER BY codigo"))
...
//	  $id = $_REQUEST['seccion'];
    ...
    $valores = array();
    if (!$cons_valores = $mysqli->query("SELECT atributo_id, id, string, `date`, `text`, `int`, `num`, leng_id FROM secciones_valores WHERE item_id = '{$id}'"))
