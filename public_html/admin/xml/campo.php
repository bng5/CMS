...
/*********************************************************************/
	...
	foreach($_POST['leng'] AS $leng_id => $nombre) $mysqli->query("INSERT INTO items_atributos_n VALUES ({$id}, ${leng_id}, '${nombre}')");
...
********************************************************************/
  ...
  $tipos = array();
  if(!$consulta = $mysqli->query("SELECT at.id, atn.nombre, at.tipo, at.subtipo FROM atributos_tipos at JOIN atributos_tipos_nombres atn ON at.id = atn.id AND atn.leng_id = '1' ORDER BY atn.nombre")) die("\n".__LINE__." mySql: ".$mysqli->error);
  if($fila = $consulta->fetch_row())
   {
	echo "
	...
	foreach($lenguajes AS $fila_lengs)
	 {
	  echo "
	    <etiqueta leng=\"{$fila_lengs[1]}\" leng_id=\"{$fila_lengs[0]}\">{$nombres[$fila_lengs[0]]}</etiqueta>";
	 }
	echo "</etiquetas>
	  <et_xhtml>${et_xhtml}</et_xhtml>
	  <extra>";
...
echo "</select></td>
	  ...
	  <tr>
	   ...
	   <td>";
	    //$marcas = array();
	  if(!$cons_marcas = $mysqli->query("SELECT id, archivo FROM imagenes_marcas ORDER BY 1")) die("\n".__LINE__." mySql: ".$mysqli->error);
	...
	elseif($tipo == 11)
	 {
	  ...
	  for($i = 0; $i < 2; $i++) echo "<li><input type=\"text\" name=\"extra_v[".$extra_indice++."]\" /></li>";
	  echo "
	    </ul></td></tr>
	  <tr>
	...
	elseif($tipo == 12)
	 {
	  ...
	  <tr>
	   ...
	   <td>
	    <ul>";
	  $extra_indice = 0;
	  if(!$consulta = $mysqli->query("SELECT id FROM campos_opciones WHERE campo_id = ${id}")) die("\n".__LINE__." mySql: ".$mysqli->error);
	  if($fila = $consulta->fetch_row())
	   {
		do
		 {
	...
	elseif($tipo == 16)
	 {
	  ...
	  if(!$consulta = $mysqli->query("SELECT id, nombre FROM monedas_nombres WHERE leng_id = 1 ORDER BY nombre")) die("\n".__LINE__." mySql: ".$mysqli->error);
	  if($fila = $consulta->fetch_row())
	   {
		...
		do
		 {
		  ...
		  if($extra == $fila[0]) echo " selected=\"selected\"";
		  echo ">{$fila[1]}</option>";
		 }while($fila = $consulta->fetch_row());
		echo "</select>";
		$consulta->close();
...
*/
  ...
  foreach($asociaciones AS $tabla => $tipo)
   {
	if(!$consulta = $mysqli->query("SELECT ads.nombre FROM ${tabla} saa JOIN admin_secciones ads ON saa.seccion_id = ads.id WHERE saa.atributo_id = ${id} ORDER BY ads.orden")) die("\n".__LINE__." mySql: ".$mysqli->error);
	if($fila = $consulta->fetch_row())
	 {
	  echo "
	<h4>${tipo}</h4>
