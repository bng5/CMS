...
elseif($cont_id == "BROWSER") {
    ...
    $navegadores_n['chrome'] = array("Google Chrome");
    ...
    foreach($array[0] AS $k => $v) {
        if(strpos($k, "firefox") === 0) {
            $navegador_grp["firefox"][$k] = $v;
            unset($array[0][$k]);
        }
        elseif(strpos($k, "msie") === 0) {
            $navegador_grp["msie"][$k] = $v;
            unset($array[0][$k]);
        }
        elseif(strpos($k, "netscape") === 0) {
            $navegador_grp["netscape"][$k] = $v;
            unset($array[0][$k]);
        }
        elseif(strpos($k, "chrome") === 0) {
            $navegador_grp["chrome"][$k] = $v;
            unset($array[0][$k]);
        }
    }
