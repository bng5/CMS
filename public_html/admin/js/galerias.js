	  ...
	  for (var i = 0; i < galerias.length; i++)
	   {
		...
		agregarAListado(contenedor, id, estado, img, orden, titulo, creada);
	   }
     }
...
function agregarAListado(contenedor, id, estado, img, orden, titulo, creada)
 {
  var tr = document.createElement('tr');
  var estadoNombre = (estado != 0)? '': 'inactivo';
  tr.className = filaEstados[estado];
  tr.estilo = filaEstados[estado];
  ...
  chbox.onclick = function()
   { selFila(chbox, filaEstados[estado]); }
...
//  ordeninput.setAttribute("id", "item"+id);
  ...
  var fecha = document.createTextNode(creada);
  td5.appendChild(fecha);
...
function actualizarOrden(campo, itemId)
 {
  if(campo.value != campo.nextSibling.value)
   {
    campo.className = 'campo_desactivado';
    loadXMLDoc('./galeria.xml?leng=1&cat='+CAT+'&id='+itemId+'&posorig='+campo.nextSibling.value+'&pos='+campo.value, cargarListado, null);
...
function eliminarPublicacion()
 {
  if(document.forms['formedicion']['id'].value.length >= 1)
   {
    loadXMLDoc('./gal_borrar_publicacion.txt?id='+document.forms['formedicion']['id'].value, publicacionEliminada, null);
   }
 }
...
function remplazarImg(campo, formulario)
 {
  if(campo.value.length > 2)
   {
	...
	var nframe = crearIframe();
	var formularioen = campo.form;
	formularioen.action = './galerias_imgsubir?id='+document.forms[formulario]['id'].value+'&frame='+nframe+'&remplazo=1';
	formularioen.target = "fr"+nframe;
	formularioen.submit();
	formularioen.reset();
...
function remplazoImg(errorno, imagenId, imagenArchivo, frame, carpeta)
 {
  filaCargando['subiendoTotal']--;
 
  if(errorno == false && imagenId != false)
   {
    ...
    filaCargando.childNodes[2].childNodes[1].src = 'imagen/img/galerias/imagenes/'+imagenArchivo;

	var remp = document.createElement('input');
    remp.setAttribute("type", "hidden");
    remp.setAttribute("name", "remplazo["+imagen.value+"]");
	remp.setAttribute("value", imagenId);
	document.forms['formedicion'].appendChild(remp);
   }
  else
   {
  ...
  if(errorno == false)
   {
    ...
    imagen.onmouseup = function()
      { desplegarImg(imagen);return false; }
