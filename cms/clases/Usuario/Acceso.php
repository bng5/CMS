  ...
  public static function acceso($campos, & $respuesta)
   {
    ...
    if($params = self::validar($campos, array('usuario' => false, 'clave' => false), $respuesta))
	 {
	  $mysqli = BaseDatos::Conectar();
	  if(!$consulta = $mysqli->query("SELECT `id`, `usuario`, `estado_id`, clave, su, leng_id FROM `usuarios` WHERE `usuario` = '".$mysqli->real_escape_string($params['usuario'])."' LIMIT 1"))
	    throw new Exception($mysqli->error, Respuesta::ERR_INTERNO);//$respuesta->agExcepcion(Respuesta::ERR_INTERNO, $mysqli->error);
	  if($fila = $consulta->fetch_assoc())
	   {
	    if($fila['clave'] == parent::encriptarClave($params['clave'], $params['usuario']))
	     {
		  $inst = new parent;
		  $inst->id = $fila['id'];
