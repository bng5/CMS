  ...
  private function campoint()
   {
    ...
    if($this->subtipo == 1)
     {
      $ret = $this->label(1)."<td>";
	  if(!$cons_vista = $this->mysqli->query($this->extra." ORDER BY 2"))
		...
		while($fila_imgs = $consulta_imgs->fetch_row())
		 {
		  ...
		  $retorno .= "<span></span><input type=\"image\" name=\"imagen[]\" src=\"icono/4/{$htmlsc}\" value=\"{$fila_imgs[0]}\" onclick=\"return false\" onmousedown=\"mover(this, event);\" onmouseup=\"desplegarImg(this)\" title=\"{$htmlsc}\" alt=\"{$htmlsc}\" />";
		 }
		$consulta_imgs->close();
