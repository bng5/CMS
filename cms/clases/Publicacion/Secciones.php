	...
	public static function publicar() {
		...
		if($seccionesListado->total) {
			...
			foreach($iterador AS $seccion) {
				$secciones[$seccion->id] = array('identificador' => $seccion->identificador, 'info' => $seccion->info, 'items' => $seccion->items, 'categorias' => $seccion->categorias, 'menu' => (int) $seccion->menu);//'id' => $seccion->id,
		...
		file_put_contents(RUTA_CARPETA.'bng5/datos/secciones.php', "<?php\n/* Archivo generado por ".get_class()." desde ".$_SERVER['REQUEST_URI']." */\n\$secciones=".var_export($secciones, true)."; \$secciones_sup=".var_export($secciones_sup, true)."; \$secciones_urls=".var_export($urls, true)."; ?>");
	}
