...
var_dump($this);
        ...
        $consultaperm = $mysqli->query("SELECT up.`area_id`, up.`item_id`, up.`permiso_id` FROM `usuarios_permisos` up JOIN secciones s ON up.item_id = s.id WHERE up.`usuario_id` = " . $this->id . " ORDER BY up.`item_id`");
    ...
    function encriptarClave($clave, $usuario, $created_date, $clave_crypt = null) {
        return crypt($clave.$created_date, $clave_crypt);

        //return sha1($clave); //md5($usuario.':'.self::REALM.':'.$clave);//sha1($cadena);
        //return crypt($str)//sha1($clave); //md5($usuario.':'.self::REALM.':'.$clave);//sha1($cadena);
    }
