...
class Berocca_Paginado {// implements Vista_Admin_iComponente
	...
	private function _enlace($pagina, $etiqueta = false) {
		...
		return '<a href="'.$this->ruta.'?'.http_build_query($this->parametros, '', '&amp;').'"><span>'.$etiqueta.'</span></a>';
	}
	/*
	...
	*/
	...
	public function __toString() { //mostrar() {
		...
		return '<div class="paginado">
            <span class="anterior'.(($this->pagina > 1) ? '">'.$this->_enlace(($this->pagina -1), $anterior) : ' inactivo"><span>'.$anterior.'</span>').'</span>
                '.implode(" ", $this->enlaces).'
                <span class="siguiente'.(($this->pagina < $this->paginas) ? '">'.$this->_enlace(($this->pagina + 1), $siguiente) : ' inactivo"><span>'.$siguiente).'</span></span>
           </div>';
		//return $retorno;
	}
}

//                        <a class="" href="#" title="atrás"><span>atrás</span></a></li>
//                        <li><a class="" href="#" title="adelante"><span>adelante</span></a></li>