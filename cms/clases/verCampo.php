...
//
	  if($this->unico == 1) $retorno .= "<img src=\"/img/trans\" onclick=\"paletaDeColores(this, this.nextSibling).mostrar()\" class=\"muestraColor\" style=\"background-color:#{$this->valores[0]['string']};\" width=\"22\" height=\"22\" alt=\"{$this->valores[0]['string']}\" />";
	  else
	   {
	   	$retorno .= "<ul>";
		$k = @each($this->valores);
	...
	elseif($this->subtipo == 4)
     {
	  ...
	  if(!$cons = $mysqli->query("SELECT id FROM campos_opciones WHERE campo_id = {$this->id}")) echo __LINE__." - ".$mysqli->error;
  ...
  private function campoint()
   {
    ...
    if($this->subtipo == 1)
     {
      $ret = $this->label(1)."<td>";
	  if(!$cons_vista = $mysqli->query($this->extra."1 ORDER BY 2")) $ret .= "Existe un error en la configuración de este campo</td>";
	  else
	   {
    ...
    elseif($this->subtipo == 3)
     {
	  ...
	  // }
      if(!$cons = $mysqli->query("SELECT id, archivo, nombre FROM `archivos` WHERE `id` = '{$valor}' ORDER BY id")) echo __LINE__." - ".$mysqli->error;
	  if($fila = $cons->fetch_row())
	   {
		$etiqueta = "Cambiar";
		if($this->unico)
    ...
    elseif($this->subtipo == 4)
     {
	  ...
	  if(!$consulta_imgs = $mysqli->query("SELECT gi.imagen_id, gi.estado, io.archivo, io.formato, gi.orden FROM galerias_imagenes gi JOIN imagenes_orig io ON gi.imagen_id = io.id WHERE gi.galeria_id = '{$this->valores[0]['int']}' ORDER BY gi.orden")) die("\n".__LINE__." mySql: ".$mysqli->error);
	  if($total_imgs = $consulta_imgs->num_rows)
	   {
		...
		if(!$cons_valores = $mysqli->query("SELECT imagen_id, atributo_id, leng_id, id, `string`, `date`, `text`, `int`, `num` FROM galerias_imagenes_valores g WHERE galeria_id = {$this->valores[0]['int']}")) echo __LINE__." - ".$mysqli->error;
